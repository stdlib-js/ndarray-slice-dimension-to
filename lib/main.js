...
function sliceDimensionTo( x, dim, stop ) {
	...
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '1lV4f', x ) );
	}
	if ( !isInteger( dim ) ) {
		throw new TypeError( format( '1lV7f', dim ) );
	}
	if ( !isInteger( stop ) ) {
		throw new TypeError( format( '1lV2z', stop ) );
	}
	if ( arguments.length > 3 ) {
		options = arguments[ 3 ];
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '1lV2V', options ) );
		}
		if ( hasOwnProp( options, 'strict' ) ) {
			if ( !isBoolean( options.strict ) ) {
				throw new TypeError( format( '1lV2o', 'strict', options.strict ) );
			}
